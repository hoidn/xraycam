...
//
//Modified by Oliver Hoidn, 3/20/2014
//---------------------------------------------------------------
...
static Point theQueue[MAX_SIZE]; // Storage array for the queue
static Point *pt; // Reference to the end of the queue
...
Point* pop()
{
    ...
    if(head == tail) return '\0';  // Return null character if queue is empty
    else
    {
        head++;
        pt = &theQueue[head % MAX_SIZE];     // Get character to return
